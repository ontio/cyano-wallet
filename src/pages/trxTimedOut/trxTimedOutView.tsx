...
export const trxTimedOutView: React.SFC<Props> = props => {
  ...
  switch (trxType) {
    default:
      message = (
        <Message>
          Your <strong>{props.trxType}</strong> transaction of <span>{props.amount}</span> <span>{props.asset}</span>{" "}
          has not completed in time. This does not mean it necessary failed. Check your balances.
        </Message>
      );
  }
